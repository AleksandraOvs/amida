...
/**
 * amida functions and definitions
 *
 ...
 */
function amida_setup()
{
	/*
		* Make theme available for translation.
		* Translations can be filed in the /languages/ directory.
		* If you're building a theme based on amida, use a find and replace
		* to change 'amida' to the name of your theme in all the template files.
		*/
	load_theme_textdomain('amida', get_template_directory() . '/languages');
	...
	register_nav_menus(
		array(
			'main_menu' => esc_html__('Primary', 'amida'),
			'footer_menu' => esc_html__('Footer menu', 'amida'),
		)
	);
...
add_action('after_setup_theme', 'amida_setup');
 ...
 */
function amida_content_width()
{
	$GLOBALS['content_width'] = apply_filters('amida_content_width', 640);
}
add_action('after_setup_theme', 'amida_content_width', 0);
 ...
 */
function amida_widgets_init()
{
	register_sidebar(
		array(
			'name'          => esc_html__('Footer-info', 'amida'),
			'id'            => 'footer-info',
			'description'   => esc_html__('Add widgets here.', 'amida'),
	...
	register_sidebar(
		array(
			'name'          => esc_html__('Footer-widget #1', 'amida'),
			'id'            => 'footer-sidebar1',
			'description'   => esc_html__('Add widgets here.', 'amida'),
			// 'before_widget' => '<div id="%1$s" class="sidebar-part %2$s">',
			// 'after_widget'  => '</div>',
...
add_action('widgets_init', 'amida_widgets_init');

/**
 * Enqueue scripts and styles.
 */
function amida_scripts()
{
	wp_enqueue_style('amida-style', get_stylesheet_uri(), array(), time());
	wp_style_add_data('amida-style', 'rtl', 'replace');
	...
	wp_enqueue_style('amida_styles', get_stylesheet_directory_uri() . '/css/style.css', array(), time());
	...
	wp_enqueue_script('amida-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true);
	...
	wp_enqueue_script('amida_scripts', get_template_directory_uri() . '/js/scripts.js', array(), _S_VERSION, true);
	wp_enqueue_script('amida_slider_scripts', get_template_directory_uri() . '/js/slider-scripts.js', array(), _S_VERSION, true);
	//wp_enqueue_script('amida_masonry_scripts', 'https://unpkg.com/masonry-layout@4/dist/masonry.pkgd.min.js">', array(), _S_VERSION, true);
...
add_action('wp_enqueue_scripts', 'amida_scripts');
